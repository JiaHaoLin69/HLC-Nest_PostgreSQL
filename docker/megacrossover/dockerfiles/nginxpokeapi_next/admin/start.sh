...
cofiguracion_final_nginx(){
    ...
    # Limpiar caché y dependencias de otros entornos (Soluciona "Cannot find module lru-cache")
    rm -rf node_modules package-lock.json .next
    npm install
    
    # Fix permisos de ejecución para next (Soluciona "next: Permission denied")
    chmod +x node_modules/.bin/next
    # 2. Compilar Next.js (El proyecto usa output: export estático)
    npm run build
